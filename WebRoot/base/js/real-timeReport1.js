...
function save() {
	if (r.test(document.getElementById('timeInterval').value)) {
		var new_stream1_minute = document.getElementById('timeInterval').value;
		// 调用jQuery提供的Ajax方法
		$
				.ajax({
					type : "POST",
					...
					url : "http://192.168.6.123:8080/pro/servlet/ChangeStream1_mimute?callback=?&new_stream1_minute="
							+ new_stream1_minute,
					data : "{}",
					dataType : "jsonp",
